...
object dataParseAndExtract{
  ...
  val tagMapped = tags.map(i => ((i.movieId,i.userId),i)).toMap
  val rat = ratings.map(i => ((i.movieId, i.userId),i)).toMap
  val tagwRating  = rat.map(ar => (tagMapped.get(ar._1).map(_.userId),tagMapped.get(ar._1).map(_.movieId),tagMapped.get(ar._1).map(_.ts),ar._2))
...
val filteredtagwRating = dataParseAndExtract.tagwRating.filter(i => i._3 != None).toList

// filteredtagwRating.map(_._3).isAfter(filteredtagwRating.map(_._4.ts))
// Yukarıdaki satırda tarihleri büyük küçük olarak kıyaslayamadığım için devam edemedim
